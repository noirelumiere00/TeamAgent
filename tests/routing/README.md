...
- 条件によらず残る外れ（シミュの作り由来）: `algo-01` / `algo-03` / `acquire-01`（「この5つの検索KW」
  ...
  聞き返した（変更前 0/2・変更後 2/6）→ 下の R5d で説明文を硬化して解消。
- R4 の 18 行（mail / calendar 系）は本ラウンドでは流していない（R4 結果欄は未実施のまま）。

### R5d（添付動画の言い換えを硬化＋SOUL 込みの本番近似シミュ）— 2026-09-24
**硬化**: `video_capture` の description と `slack_file` 引数に「『さっき貼った動画』『上の動画』と言われたら、
添付が見えなくても聞き返さずに呼ぶ（探すのはサーバ・無ければ案内を返す）」を追記。
行を 2 つ追加（`vcapture-03`「上の動画の0:10」/ `vcapture-04`「このスレに貼った動画、2:05」）＝計 20 行。

**本番近似シミュ**: 本番の外側ループは Haiku 4.5 で、**SOUL.md を system prompt として読んだうえで**
ツールを選ぶ（`openclaw.config.json5` の `model`）。R1〜R5c のルーター（name+description だけ）では
SOUL の効き目が測れないので、Haiku に「SOUL.md を system prompt とみなす＋tools.json＋DM の 1 通目」を
渡し、最初の一手（ツール呼び出し or 返信文）を出させた。変更前 = origin/dev の SOUL＋説明文、
変更後 = PR #441 の SOUL＋本 PR の説明文。

| シミュ | 条件 | 本数 | 一致（/20） | YouTube 分析 3 行 | YouTube 切り出し | 添付動画 3 行 | `forbid` 違反 |
|---|---|---|---|---|---|---|---|
| ルーター（name+description） | 最終形 | 3 | 17 / 18 / 17 | 9 / 9 | 3 / 3 | 9 / 9 | 0 |
| SOUL 込み | 変更前 | 2 | 17 / 10 | 3 / 6 | 1 / 2 | 3 / 6 | 0 |
| SOUL 込み | 変更後 | 3 | 18 / 18 / 17 | **9 / 9** | **3 / 3** | **9 / 9** | 0 |

- **本番の断りを再現した**: 変更前の 2 本目は「YouTube の動画分析は取得元にブロックされています。動画ファイルを
  添付いただければ対応できます。」とツールを呼ばずに返した（本番の応答とほぼ同文）。YouTube の分析 3 行・
  切り出し 1 行・添付動画 2 行が同じ run で揃って断り／聞き返しに倒れた。変更後の 3 本では 0 件。
- 変更前の 1 本目は全部正しく呼んだ＝事故は確率的に起きる（本番の 1 回もその一例）。n が小さいので率の比較は
  しないが、変更後 3 本 × YouTube・添付 7 行 = 21 件で断り／聞き返しは 0。
- 残る外れ: `algo-01` / `algo-03` / `acquire-01`（KW の中身が発話に無い）・`vanalysis-02`（`<url>×3`）は
  条件と関係なく聞き返しか別ツールに倒れる（ルーター最終形 A で 1 回 `video_algorithm`。forbid 外）。
  `boundary-02` は SOUL 込み変更前の 1 本目で 1 回 `tiktok_acquire`。
- 限界: シミュのエージェントはそれ自体の system prompt の上で SOUL を「読む」ので、OpenClaw の実行時
  （bootstrap 注入・IDENTITY.md・会話履歴）と同一ではない。最終確認は便の後の実機 DM で行う。
