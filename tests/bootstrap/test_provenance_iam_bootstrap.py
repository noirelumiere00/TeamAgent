...
def test_seed_is_temporary_mfa_sts_only_and_denies_dangerous_paths() -> None:
    ...
    assert "arn:aws:iam::aws:policy/ReadOnlyAccess" in body
    ...
    assert "iam:PolicyARN: arn:aws:iam::aws:policy/PowerUserAccess" in body
...
class _RetirementRunner:
    ...
    def run(
        ...
        *,
        ...
        if "get-role" in call or "get-policy" in call:
            return BOOTSTRAP.CommandResult(
                stdout=b"",
...
def test_runtime_prerequisites_are_main_owned_and_root_must_assume_sts() -> None:
    ...
    assert 'resource "aws_iam_role_policy_attachment" "runtime_automation_power_user"' in body
    ...
    assert '"iam:CreateAccessKey"' in body
    assert '"iam:CreateInstanceProfile"' in body
    assert '"iam:AddRoleToInstanceProfile"' in body
    assert '"arn:aws:iam::718959508629:instance-profile/teamagent-*"' in body
    ...
    assert "depends_on = [aws_iam_role_policy.runtime_automation_control_plane]" in body
    assert '"codebuild:StartBuild"' in body
    assert '"ecr:PutImage"' in body
    assert '"kms:Sign"' in body
...
def test_entrypoints_never_dispatch_build_or_release_from_bootstrap_session() -> None:
    ...
    assert 'exec python3 -I "$HELPER"' in bootstrap
    ...
    assert runtime.index("unset AWS_CONFIG_FILE AWS_SHARED_CREDENTIALS_FILE") < runtime.index(
        'exec bash "$SCRIPT_DIR/terraform_runtime_guard.sh" "$@"'
    )
    for command in (
        "snapshot",
    ...
    assert 'exec bash "$SCRIPT_DIR/terraform_runtime_guard.sh" "$@"' in runtime
...
def test_blocked_contract_cannot_mint_root_provenance_session(tmp_path: Path) -> None:
    ...
    assert "not ready" in completed.stderr or "release is blocked" in completed.stderr
