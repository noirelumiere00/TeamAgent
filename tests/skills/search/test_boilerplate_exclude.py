"""SearchSkill._retrieve への boilerplate 除外（env-gate）の統合テスト。

契約:
- env OFF（既定 / 未設定）: search_similar_new_schema は exclude_boilerplate=False で呼ばれる
  ＝現行と完全一致（テンプレ除外は一切起きない）
- env ON（BOILERPLATE_EXCLUDE_SEARCH=1/true/yes）: exclude_boilerplate=True で呼ばれる
- env 読み取りは skill 側（__init__）で 1 回（dedup の env と同じ流儀）
- 渡すだけ。テンプレ判定（指紋＋出現 document 数）は pgvector 側（SQL）の責務。
...
def _exclude_kwarg(pg: MagicMock) -> bool:
    """最初の search_similar_new_schema 呼び出しに渡った exclude_boilerplate を返す。
    ...
    return bool(call.kwargs.get("exclude_boilerplate", False))


def test_env_off_passes_false(monkeypatch: pytest.MonkeyPatch) -> None:
    # 未設定 → exclude_boilerplate=False（現行と完全一致）
    ...
    for call in pg.search_similar_new_schema.call_args_list:
        assert call.kwargs.get("exclude_boilerplate", False) is False


def test_env_on_passes_true(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("BOILERPLATE_EXCLUDE_SEARCH", "1")
    pg = _pgvector(_hits())
    _skill(pg).run(input=SearchInput(query="ユニーの提案", top_k=5), ctx=SkillContext())
    assert _exclude_kwarg(pg) is True
    for call in pg.search_similar_new_schema.call_args_list:
        assert call.kwargs.get("exclude_boilerplate") is True
...
def test_env_truthy_variants_enable(monkeypatch: pytest.MonkeyPatch, raw: str) -> None:
    monkeypatch.setenv("BOILERPLATE_EXCLUDE_SEARCH", raw)
    pg = _pgvector(_hits())
...
def test_env_read_at_construction(monkeypatch: pytest.MonkeyPatch) -> None:
    # __init__ で 1 回読んで self に保持（dedup の env と同じ流儀）
    monkeypatch.setenv("BOILERPLATE_EXCLUDE_SEARCH", "yes")
    skill = _skill(_pgvector(_hits()))
    assert skill._exclude_boilerplate is True
    # 構築後に env を変えても保持値は不変（再読込しない）
    monkeypatch.setenv("BOILERPLATE_EXCLUDE_SEARCH", "0")
    assert skill._exclude_boilerplate is True
...
def test_client_boost_call_also_carries_flag(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("BOILERPLATE_EXCLUDE_SEARCH", "true")
    pg = _pgvector(_hits())
