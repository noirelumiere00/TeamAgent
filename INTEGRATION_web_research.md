...
| env | 誰が設定 | 値 | 備考 |
| --- | --- | --- | --- |
| `USE_WEB_RESEARCH_TOOL` | tf `use_web_research_tool` | `true` | 既定 false。ON でツールが MCP に出る。⚠️ 値の真実源は **git 管理外の tfvars**。「全員開放」のうち git で読めるのは下段の allowlist だけで、フラグが実際に ON かは tfvars 実物か本番 taskdef の env でしか確認できない |
