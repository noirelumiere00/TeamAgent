"""x-buzz（X発話量 効果測定④）ディスパッチャ Lambda。
SQS(x-buzz-jobs)からジョブJSONをデキューし、ECS RunTask(Fargate)を起動する。
ジョブ本文(JSON文字列)を containerOverrides の env X_JOB_JSON にそのまま注入する。
...
def handler(event, context):
    ...
    for rec in event.get("Records", []):
        ...
        try:
            ...
            print(f"[x-dispatch] skip invalid message: {e}")
            continue
        ...
        print(f"[x-dispatch] started job={spec.get('job_id')} task={task_arn}")
