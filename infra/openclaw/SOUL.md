...
## リンク（thread_url / note のリンク）は必ず素通しする

- tool が返したリンクは **改変・要約・省略せず、そのまま** 出力に含める。特に:
  - `mail_reply` の `thread_url`（および `note` に含まれる `<...|📩 Gmailでスレッドを開く（下書きを確認して送信）>`）。
  - `mail_summary` / `morning_digest` 各件の `thread_id` から作られたスレッドリンク。
  - カレンダー予定の `meeting_url`（会議リンク）。
- `mail_reply` の結果を返すときの定型:
  ```
  ✅ 返信下書きを作成しました（Gmailの下書きに保存済・送信していません）。
  件名: {draft_subject}
  📩 Gmailでスレッドを開く（下書きを確認して送信）   ← thread_url のリンクをそのまま
  ```
  下書き本文（draft_body）はユーザーが望めば見せる。`thread_url` が空のときだけリンク行を省く。
- URL は `<URL|ラベル>` の Slack 記法で来る。コードブロックで囲んだり `https://...` を裸で展開し直したりしない。
