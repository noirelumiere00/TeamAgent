...
SAFE_TERMINAL_CLASSIFICATIONS = frozenset({"INITIAL_NEW", "PREVIOUS_OLD", "UNKNOWN"})
RECOVERY_RESULTS = frozenset({"PASSED", "FAILED", "NOT_ATTEMPTED", "NOT_REQUIRED"})
...
_KMS_KEY_ARN_RE = re.compile(rf"arn:aws:kms:{REGION}:{ACCOUNT_ID}:key/[A-Za-z0-9-]{{1,256}}")
_PRINCIPAL_ARN_RE = re.compile(
    rf"arn:aws:(?:iam|sts)::{ACCOUNT_ID}:"
    r"(?:role|user|assumed-role)/[A-Za-z0-9+=,.@_/-]{1,512}"
)
_IAM_ROLE_ARN_RE = re.compile(rf"arn:aws:iam::{ACCOUNT_ID}:role/[A-Za-z0-9+=,.@_/-]{{1,512}}")
...
_TERRAFORM_ADDRESS_RE = re.compile(r"aws_ecs_task_definition\.[a-z][a-z0-9_]*(?:\[[0-9]+\])?")
...
_CONTENT_TYPE_RE = re.compile(r"[a-z0-9][a-z0-9.+-]{0,126}/[A-Za-z0-9][A-Za-z0-9.+_-]{0,126}")
...
def _exact_object(
    ...
    *,
    ...
    if missing or unknown:
        raise DrillEvidenceError(f"{label} schema mismatch: missing={missing}, unknown={unknown}")
    return item
...
def canonical_drill_body_bytes(value: dict[str, Any]) -> bytes:
    ...
    for key in ("canonical_sha256", "signature", "immutable_object"):
        if key not in integrity:
            raise DrillEvidenceError(f"drill.integrity.{key} is required for canonical projection")
        del integrity[key]
    try:
...
def _validate_redownload(
    ...
    *,
    ...
    if requested != version_id or returned != version_id:
        raise DrillEvidenceError(f"{label} must request and return the locator's exact VersionId")
    if redownload_sha != sha256 or redownload_size != size:
        raise DrillEvidenceError(f"{label} SHA-256 and size must exactly match the locator")
...
def _validate_locator(
    ...
    *,
    ...
    if not _S3_BUCKET_RE.fullmatch(bucket) or ".." in bucket or ".-" in bucket or "-." in bucket:
    ...
    if retain_until <= aggregate_completed_at:
        raise DrillEvidenceError(f"{label}.retain_until must be after drill completion")
    _kms_key_arn(
    ...
    if signer["algorithm"] != SIGNING_ALGORITHM:
        raise DrillEvidenceError(f"{label}.signer.algorithm must be {SIGNING_ALGORITHM}")
...
def _validate_automation_principal(
    ...
    *,
    ...
    if principal["account_id"] != ACCOUNT_ID:
        raise DrillEvidenceError(f"{label}.account_id is not the fixed AWS account")
    _pattern(
        principal["arn"],
        _PRINCIPAL_ARN_RE,
        label=f"{label}.arn",
    ...
    if principal["arn"] != TRUSTED_AUTOMATION_ARN:
        raise DrillEvidenceError(f"{label}.arn is not the exact trusted automation caller")
    _text(principal["user_id"], label=f"{label}.user_id", maximum=512)
    return principal
...
def _validate_control(
    ...
    *,
    ...
    if not verified_at <= started_at <= completed_at:
        raise DrillEvidenceError("drill.control timestamps must be monotonically nondecreasing")
    _literal_integer(
...
def _validate_actors(value: Any) -> dict[str, Any]:
    ...
    if not automation_values:
        raise DrillEvidenceError("drill.actors.automation_principals must not be empty")
    ...
    automation_identities = [_automation_principal_identity(item) for item in automation]
    if len(automation_identities) != len(set(automation_identities)):
        raise DrillEvidenceError("drill.actors.automation_principals must be unique")
    if automation_identities != sorted(automation_identities):
        raise DrillEvidenceError("drill.actors.automation_principals must be canonically sorted")
    if any(
        initiating["arn"] == item["arn"] and initiating["user_id"] == item["user_id"]
        for item in automation
    ):
        raise DrillEvidenceError("drill initiating and automation principals must be distinct")
    ...
    if len(approval_values) != 2:
        raise DrillEvidenceError("drill.actors.approvals must contain exactly two approvals")
    ...
    for index, raw_approval in enumerate(approval_values):
        ...
        approval_ids.append(_uuid4(actor_approval["approval_id"], label=f"{label}.approval_id"))
        _principal_arn(
            actor_approval["approved_by_arn"],
            label=f"{label}.approved_by_arn",
        )
        if actor_approval["approved_by_arn"] != APPROVAL_APPROVED_BY_ARN:
            raise DrillEvidenceError(f"{label}.approved_by_arn is not the fixed approval caller")
...
def _validate_scope(value: Any) -> dict[str, Any]:
    ...
    if pipelines != sorted(set(pipelines)):
        raise DrillEvidenceError("drill.scope.pipelines must be unique and canonically sorted")
    ...
    for index, raw_subject in enumerate(raw_subjects):
        ...
        if previous == initial_new:
            raise DrillEvidenceError(f"{label} previous and initial-new digests must differ")
        subjects.append(subject)
        subject_identities.append((pipeline, name))
    if subject_identities != sorted(set(subject_identities)):
        raise DrillEvidenceError("drill.scope.subjects must be unique and canonically sorted")
    if set(pipelines) != {pipeline for pipeline, _ in subject_identities}:
        raise DrillEvidenceError("drill.scope.pipelines must exactly cover subject pipelines")

    subjects_by_identity = {
        identity: subject for identity, subject in zip(subject_identities, subjects, strict=True)
    }
    ...
    if consumer_ids != sorted(set(consumer_ids)):
        raise DrillEvidenceError("drill.scope.resources must be unique and sorted by consumer_id")
    if len(terraform_addresses) != len(set(terraform_addresses)):
        raise DrillEvidenceError("drill.scope.resources terraform addresses must be unique")
    if referenced_subjects != set(subject_identities):
        raise DrillEvidenceError("drill.scope.resources must cover every scoped subject")
    ...
    previous_digests = {
        (subject["pipeline"], subject["name"]): subject["previous_digest"] for subject in subjects
    }
    initial_new_resources = [
        {
            ...
            "digest": initial_new_digests[(resource["pipeline"], resource["subject"])],
        }
    ...
    previous_resources = [
        {
            ...
            "digest": previous_digests[(resource["pipeline"], resource["subject"])],
        }
        for resource in resources
    ]
    return {
...
def _validate_subject_list(
    ...
    *,
    ...
    if len(entries) != len(expected_subjects):
        raise DrillEvidenceError(f"{label} must contain every expected subject exactly once")
    for index, (raw_entry, scoped) in enumerate(zip(entries, expected_subjects, strict=True)):
        ...
        for key in ("pipeline", "name", "release_repository"):
            if entry[key] != scoped[key]:
                raise DrillEvidenceError(f"{entry_label}.{key} does not match drill.scope")
...
def _validate_snapshot(
    ...
    *,
    ...
    actual_digests = {
        (subject["pipeline"], subject["name"]): subject["digest"] for subject in subjects
    }
    ...
    for index, (raw_resource, scoped) in enumerate(zip(resources, scope["resources"], strict=True)):
        ...
        for key in (
            ...
            if resource[key] != scoped[key]:
                raise DrillEvidenceError(f"{resource_label}.{key} does not match drill.scope")
        ...
        subject_digest = actual_digests[(resource["pipeline"], resource["subject"])]
        if digest != subject_digest:
            raise DrillEvidenceError(f"{resource_label}.digest differs from its subject digest")
    return snapshot
...
def _validate_baseline(
    ...
    *,
    ...
    if live_snapshot["snapshot"] != scope["initial_new"]:
        raise DrillEvidenceError("drill.baseline.live_snapshot must be the exact initial-new state")
    return baseline
...
def _validate_release_authorizations(
    ...
    *,
    ...
    if len(authorizations) != len(scope["pipelines"]):
        raise DrillEvidenceError(f"{label} must contain exactly one authorization per pipeline")
    for index, (raw_authorization, pipeline) in enumerate(
        ...
        if authorization["drill_id"] != drill_id:
            raise DrillEvidenceError(f"{authorization_label}.drill_id does not bind this drill")
        ...
        if authorization["channel"] != channel:
            raise DrillEvidenceError(f"{authorization_label}.channel does not bind its leg")
        subjects = _validate_subject_list(
        ...
        expected_subjects = [
            subject for subject in target["subjects"] if subject["pipeline"] == pipeline
        ]
        if subjects != expected_subjects:
            raise DrillEvidenceError(f"{authorization_label}.subjects do not bind the leg target")
        _timestamp(
        ...
        if authorization["release_approved_by_arn"] != APPROVAL_APPROVED_BY_ARN:
            raise DrillEvidenceError(
                f"{authorization_label}.release_approved_by_arn is not the fixed approval caller"
            )
        ...
        if locator["sha256"] != receipt_sha256:
            raise DrillEvidenceError(f"{authorization_label}.receipt_sha256 must match its locator")
        locators.append(locator)
...
def _validate_plan(
    ...
    *,
    ...
    _sha256(plan["sha256"], label=f"{label}.sha256")
    receipt_sha = _sha256(
        plan["receipt_sha256"],
    ...
    expected_resources = [resource["terraform_address"] for resource in scope["resources"]]
    ...
    if locator["sha256"] != receipt_sha:
        raise DrillEvidenceError(f"{label}.receipt_sha256 must match its exact receipt locator")
    locators.append(locator)
    return plan
...
def _validate_approval(
    ...
    *,
    ...
    expected_confirmation = f"APPROVE {drill_id} {approval['action']} {plan_sha256}\n".encode()
    if hashlib.sha256(expected_confirmation).hexdigest() != approval["approval_text_sha256"]:
        raise DrillEvidenceError(
    ...
    if approved_at >= expires_at:
        raise DrillEvidenceError(f"{label}.release_approval expiration must follow approval")
    _pattern(
        release["approved_by"],
        _IAM_ROLE_ARN_RE,
    ...
    if release["approved_by"] != APPROVAL_APPROVED_BY_ARN:
        raise DrillEvidenceError(
            f"{label}.release_approval.approved_by is not the fixed approval caller"
        )
    decision = _text(
        release["decision"],
    ...
    if not decision.startswith("APPROVED: "):
        raise DrillEvidenceError(f"{label}.release_approval.decision is not an approval")
    _sha256(
        release["forced_gate_sha256"],
        label=f"{label}.release_approval.forced_gate_sha256",
    )
    if release["pipeline"] != "mcp":
        raise DrillEvidenceError(f"{label}.release_approval.pipeline must be mcp")
    _pattern(
    ...
    expected_payload_key = (
        f"approval-records/mcp/{release['source_commit']}/{payload['sha256']}.json"
    )
    if (
    ...
    if locator["sha256"] != receipt_sha:
        raise DrillEvidenceError(f"{label}.receipt_sha256 must match its exact locator")
    locators.append(locator)
    return approval
...
def _validate_apply(
    ...
    *,
    ...
    if automation_identity_locator["sha256"] != automation_identity_sha:
        raise DrillEvidenceError(f"{label}.automation_identity_sha256 must match its locator")
    locators.append(automation_identity_locator)
    if result == "PASSED":
        if serial_after <= serial_before:
            raise DrillEvidenceError(f"{label} successful apply must advance the Terraform serial")
        if state != target:
            raise DrillEvidenceError(
...
def _validate_run_task_health(
    ...
    *,
    ...
    if health["apply_attempt_id"] != apply_attempt_id:
        raise DrillEvidenceError(f"{label}.apply_attempt_id does not bind its apply")
    ...
    if len(matching_resources) != 1 or matching_resources[0]["consumer_id"] != "canary":
        raise DrillEvidenceError(f"{label}.task_definition_arn is not the leg target canary")
    image = _pattern(
        health["image"],
        _ECR_IMAGE_RE,
        label=f"{label}.image",
    ...
    if len(matching_subjects) != 1 or image != (
        f"{ACCOUNT_ID}.dkr.ecr.{REGION}.amazonaws.com/"
        f"{matching_subjects[0]['release_repository']}@"
        f"{matching_subjects[0]['digest']}"
    ):
        raise DrillEvidenceError(f"{label}.image does not bind its target subject digest")
    ...
    if task["stopped_reason_code"] != "EssentialContainerExited":
        raise DrillEvidenceError(f"{label}.task.stopped_reason_code is not successful")
...
def _validate_dm_qa(
    ...
    *,
    ...
    if result == "PASSED" and (
        expected_mcp != [mcp_task_definition] or openclaw_family != "teamagent-dev-openclaw"
    ):
        raise DrillEvidenceError(
    ...
    expected_key = f"forced-rollback-drills/{apply_attempt_id}/dm-qa/result.json"
    if (
        locator["bucket"] != "teamagent-dev-openclaw-rollout-evidence"
        or locator["key"] != expected_key
    ):
        raise DrillEvidenceError(f"{label}.locator does not bind the exact apply DM QA evidence")
    locators.append(locator)
    return dm_qa
    ...
    *,
    ...
    if attempted:
        if result not in {"PASSED", "FAILED"}:
            raise DrillEvidenceError(f"{label}.result must record an attempted recovery outcome")
        _timestamp(
        ...
        if result not in {"NOT_ATTEMPTED", "NOT_REQUIRED"}:
            raise DrillEvidenceError(f"{label}.result is inconsistent with attempted=false")
...
def _validate_leg(
    ...
    *,
    ...
    if (leg["ordinal"], leg["name"], leg["channel"]) != expected:
        raise DrillEvidenceError(f"{label} must be ordinal/name/channel {expected!r}")
    ...
    if require_exact_transition and (leg_from != required_source or leg_to != required_target):
        raise DrillEvidenceError(f"{label} endpoints are not the exact required transition")
    ...
    if (
        ...
        raise DrillEvidenceError(
            f"{label}.approval must use confirmation/action {expected_confirmation!r}"
        )
    release_approval = approval["release_approval"]
    for authorization_index, authorization in enumerate(authorizations):
        if (
            authorization["release_approval_id"] != release_approval["approval_id"]
            or authorization["release_approved_by_arn"] != release_approval["approved_by"]
        ):
            raise DrillEvidenceError(
        ...
        issued_at = _timestamp(
            authorization["issued_at_utc"],
            label=(f"{label}.release_authorizations[{authorization_index}].issued_at_utc"),
        )
        ...
        if not release_approved_at <= issued_at < release_expires_at:
            raise DrillEvidenceError(f"{label} release authorization is outside approval validity")
    ...
    if ecs["locator"] != apply["locator"] or run_task_health["locator"] != apply["locator"]:
        raise DrillEvidenceError(
    ...
    if not (
        ...
        and started_at <= plan_created_at <= confirmed_at <= apply_started_at <= apply_completed_at
    ...
    if result == "PASSED" and not proof_passed:
        raise DrillEvidenceError(f"{label}.result cannot be PASSED when a required proof failed")
...
def _validate_safe_terminal_state(
    ...
    *,
    ...
    if expected_snapshot is not None and live_snapshot["snapshot"] != expected_snapshot:
        raise DrillEvidenceError("drill.safe_terminal_state classification and snapshot disagree")
    if classification == "UNKNOWN" and steady:
        raise DrillEvidenceError("drill.safe_terminal_state UNKNOWN cannot be marked steady")
    return terminal
...
def _validate_artifact_manifest(
    ...
    *,
    ...
    if manifest_identities != sorted(manifest_identities):
        raise DrillEvidenceError("drill.artifact_manifest must be sorted by bucket/key/VersionId")
    if len(manifest_identities) != len(set(manifest_identities)):
        raise DrillEvidenceError("drill.artifact_manifest must not contain duplicate locators")
...
def _validate_integrity(
    ...
    *,
    ...
    if integrity["signing_algorithm"] != SIGNING_ALGORITHM:
        raise DrillEvidenceError(f"drill.integrity.signing_algorithm must be {SIGNING_ALGORITHM}")
    ...
    if immutable["sha256"] != expected_sha or immutable["size"] != len(canonical_body):
        raise DrillEvidenceError(
...
def _validate_cross_invariants(
    *,
    ...
    if (
        ...
        raise DrillEvidenceError("both plans and applies must bind the baseline Terraform lineage")
    if (
        leg1["plan"]["terraform_serial"] != baseline_serial
        or leg1["apply"]["terraform_serial_before"] != baseline_serial
        or leg2["plan"]["terraform_serial"] != leg1["apply"]["terraform_serial_after"]
        or leg2["apply"]["terraform_serial_before"] != leg1["apply"]["terraform_serial_after"]
    ):
        raise DrillEvidenceError(
            "restore plan must be based on the rollback-complete Terraform serial"
        )
    if leg2["plan_created_at"] < leg1["completed_at"]:
        raise DrillEvidenceError("restore plan must be created after rollback leg completion")
    if leg1["plan"]["sha256"] == leg2["plan"]["sha256"]:
        raise DrillEvidenceError("each leg must use a fresh, distinct plan")
    if leg1["apply"]["apply_attempt_id"] == leg2["apply"]["apply_attempt_id"]:
        raise DrillEvidenceError("each leg must use a fresh apply attempt ID")

    authorization_ids = [
        authorization["authorization_id"] for leg in legs for authorization in leg["authorizations"]
    ]
    intent_ids = [
        authorization["deployment_intent_id"]
        for leg in legs
    ...
    for leg in legs:
        for authorization in leg["authorizations"]:
            if authorization["drill_id"] != drill_id:
                raise DrillEvidenceError("every release authorization must bind the drill ID")
    ...
    for index, (actor_approval, leg_approval) in enumerate(
        ...
        if actor_approval["approved_by_arn"] != leg_approval["release_approval"]["approved_by"]:
            raise DrillEvidenceError(
                f"leg {index + 1} release approver differs from actors.approvals"
            )
        if leg_approval["drill_id"] != drill_id:
            raise DrillEvidenceError(f"leg {index + 1} approval does not bind the drill ID")
        if leg_approval["plan_sha256"] != legs[index]["plan"]["sha256"]:
            raise DrillEvidenceError(f"leg {index + 1} approval does not bind its exact plan SHA")
    actual_automation = sorted(
        {
            _automation_principal_identity(leg["apply"]["automation_principal"]): leg["apply"][
                "automation_principal"
            ]
            for leg in legs
        }.values(),
    ...
    if not (
        ...
        raise DrillEvidenceError(
            "drill, leg, and safe-terminal timestamps must be monotonically nondecreasing"
        )

    if status == "PASSED":
        if leg1["value"]["from"] != initial_new:
            raise DrillEvidenceError("leg1.from must equal the baseline initial-new state")
        ...
        if leg2["value"]["to"] != initial_new:
            raise DrillEvidenceError("leg2.to must equal the baseline initial-new state")
        if any(leg["result"] != "PASSED" for leg in legs):
            raise DrillEvidenceError("a drill containing any FAILED leg can never be PASSED")
        if (
            leg1["apply_started_at"] - control["verified_at"]
        ).total_seconds() > MAX_START_DELAY_SECONDS:
            raise DrillEvidenceError("rollback apply exceeded max_start_delay_seconds")
        if (
            control["started_at"] - control["verified_at"]
        ).total_seconds() > MAX_START_DELAY_SECONDS:
            raise DrillEvidenceError("drill start exceeded max_start_delay_seconds")
        if (leg2["started_at"] - leg1["completed_at"]).total_seconds() > MAX_OLD_DWELL_SECONDS:
            raise DrillEvidenceError("previous-old dwell exceeded max_old_dwell_seconds")
        for index, leg in enumerate(legs):
            recovery = leg["recovery"]
            if (
                recovery["attempted"]
                or recovery["result"] != "NOT_REQUIRED"
                or recovery["last_exact_confirmed_digests"] != leg["value"]["to"]["subjects"]
            ):
                raise DrillEvidenceError(f"PASSED leg {index + 1} cannot contain recovery activity")
        if leg2["apply"]["state"] != initial_new or leg2["ecs"]["live_snapshot"] != initial_new:
            raise DrillEvidenceError(
        ...
        if (
            ...
            raise DrillEvidenceError("PASSED requires a steady exact INITIAL_NEW terminal state")
    else:
        if not any(leg["recovery"]["result"] != "NOT_REQUIRED" for leg in legs):
            raise DrillEvidenceError(
...
def validate_drill_evidence(
    ...
    for key in (
        ...
        if drill["control"][key] != expected_bindings[key]:
            raise DrillEvidenceError(f"drill.control.{key} differs from the trusted binding")
    actors = _validate_actors(drill["actors"])
    scope = _validate_scope(drill["scope"])
    if drill["scope"] != expected_bindings["scope"]:
        raise DrillEvidenceError("drill.scope differs from the trusted registry/receipt closure")
