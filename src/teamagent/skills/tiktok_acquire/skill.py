...
class TikTokAcquireSkill(BaseSkill[TikTokAcquireInput, TikTokAcquireOutput]):
    ...
    description: ClassVar[str] = (
        "商材名/KW群のTikTok上位動画を「動画本体(mp4)込みで一括取得し、提案・マルチモーダル分析の"
        "素材としてストックする」ツール。各KW最大30本の指標+サムネを集め、上位N本(既定6)はmp4本体もS3に保存する。"
        "トリガー=「○○でTikTok取得して/動画も保存して/保存率上位の動画も/素材集めて/ストックして/提案用に集めて」。"
        "分析や要約はせず素材を貯めるだけ（構造分析レポートは video_algorithm、"
        "その場で見るだけの軽い検索・横断分析は tiktok_search）。"
        "非同期で即 job_id を返すので、結果は tiktok_acquire_status で受け取る。"
    )
...
class TikTokAcquireStatusSkill(BaseSkill[TikTokAcquireStatusInput, TikTokAcquireStatusOutput]):
    ...
    description: ClassVar[str] = (
        "tiktok_acquire が返した job_id の取得結果をポーリング照会する"
        "（tiktok_acquire 専用の後工程で、単独の入口にはしない）。"
        "done なら posts/サムネ/動画(mp4)を署名URLとS3キーで返す"
        "（url=人向け / s3_key=機械処理用）。"
    )
