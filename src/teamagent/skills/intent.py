...
# 「連携」「メール連携」「連携して」「接続して」「connect」等で本人専用の認可リンクを返す。
# 「連携事例を教えて」等の検索を奪わないため、(a)連携系語＋意図動詞 (b)明示プレフィックス
# (c)メッセージ全体が連携系の語だけ（単独「連携」など）のいずれかで発火する。
_CONNECT_RE = re.compile(
    r"(?:メール|gmail|google|グーグル|アカウント|account)\s*(?:連携|連動|接続)"
    r"|(?:連携|接続|コネクト|連動)\s*(?:して|したい|する|お願い|よろしく|セットアップ)"
    r"|連携リンク|認可リンク|連携を?開始"
    # メッセージ全体が連携系の語だけ（単独「連携」「接続」「connect」等。前後の記号/絵文字は許容）
    r"|^\s*(?:連携|再連携|接続|コネクト|connect|reconnect)\s*[!！。、.…〜~ー♪🙏👍✅]*\s*$",
    re.IGNORECASE,
)
