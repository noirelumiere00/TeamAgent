...
⚠️ 2026-09-18: Gemini 3.5 系（flash-lite/flash）は「URL を書くな」の一文だけだと
「検索して裏取りする必要すらない」と拡大解釈し、Google 検索そのものをスキップすることが
あった（2.5-flash は同じ文言でも常に検索する）。そのため「必ず検索して根拠づけよ」を
「URL を書くな」より前に明示している。この順序・両方の文を削らないこと。
"""
...
- 必ず Google 検索ツールを使い、要約の各記述は検索で得たページに根拠づけてください\
（グラウンディングの引用メタデータを必ず付けること）。
- ただし出力の本文には URL・リンク・脚注番号・出典表記を書かないでください（出典は\
システムが groundingMetadata から機械的に付けます）。
