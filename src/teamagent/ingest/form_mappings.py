...
- 写像しない列: ファイルをアップ (Slack file URL・運用列)・フリーコメント (長文自由記述で
  ...
  タイムスタンプ / ドライブ格納 / 保管先フォルダID記録 (GAS 運用列)。
...
_KNOWLEDGE_LABEL_TO_METADATA_KEY: dict[str, str] = {
    ...
    # 実 dump (2026-07-15) の値域は 提案107/社内共有情報18/レポート13/クロージング2/AI活用2/
    # 効果体感2/オリエン資料1 の **素の値**（"01_提案" のような NN_ 接頭は付かない）。
    # ※「保管先フォルダ」列は現行シートに存在しない（あるのは 旧_保管先フォルダ で、値は
    #   "20250624_東京都_…" の date_client 形式）。99_一次倉庫 の除外は pipeline._ingest_gsheet が
    #   gdrive と同一 regex で行を落とす形で実装済み（現行 dump では 99_ は 0 件＝将来行への保険）。
