...
- 残miss: case12「飲料メーカー」（カテゴリ）・case15「BANT A 案件一覧」（集約）。**集約モード(既存)はA/Bで case15 回復せず**（抽出器が当該phrasingに不発火）。→ **検証で効いた精度施策は client-boost(+4pp)のみ**。残2件は固有名詞でなく安価レバー対象外＝gold setはほぼ天井(80-88%)。次の精度ジャンプは #5忠実性(別軸)/multi-query(#6・低ROI)。残フェーズ: Phase 6c（人間ゲート後）、Phase 3（ゲート①）、#5忠実性eval、（任意）複数seed・Phase 5。今ターン精度実験コスト累計≈$2.1。
