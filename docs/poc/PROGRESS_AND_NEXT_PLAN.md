...
## 0. 現在地（ひと言）
**Phase 0→1→2 ライブ達成。2026-06-03 に「実コーパスからの grounded 出力」を実証（commit `9d415d6`）。** 実RDS(9420 chunks)から chunk_id/Drive URL/見積数値つきの根拠付き提案を、実Bedrockで生成できることを確認（hit 10/5/5・top_score 0.49-0.86・is_error=False・$0.106・ハルシ無し）。**さらに Phase 6（Mail/Drive横断）の 6a設計・6b実装・6d配線まで完了（commit `f69bcbd`、`mail_constraints` スキル＝『MailのNG→別案差替』の中核、課金0・pytest 10 passed・既定OFF）**。**Phase 4 完了（commit `0b7bd2b`＋実eval 2026-06-03）**：gold set 10本＋決定的採点（pytest 27緑）、実Bedrock eval で **実質 8/8（100%）ルーティング・$0.76**、唯一の不合格はゴールド校正欠陥を検出（proposal_review の self-grounding）＝評価が機能。**精度向上施策＝`min_relevance` 2段階化（strict guard + relax fallback・低信頼マーク）も実装（commit `3d3d647`・追加コスト0・後方互換・pytest 33緑）**＝『0件 vs 捏造』の二択を解消する任意ガード。**A/B実測済（2026-06-03・$0.66）**: gold set 25件で top-5 hit **88%→88%（改善せず）**。ただし**ネガ捏造抑止は完全維持＋fallback=0.25の校正が正解と実証**（0.15ならネガ退行＝批判の警告どおり）。失敗3件は閾値でなく**リコール問題**（正解chunk不在）。→ **評価が『未検証改善の早すぎる採用』を阻止**＝Phase4が機能。2段階は安全・default OFFの任意ガードとして温存。
- **★クライアント名ブースト（#2・commit `5c53a3f`）はA/Bで +4pp 実証（top5 76%→80%・2026-06-03）**: 固有名詞 case10「ユニー」を回復（0→rank1）、非クライアントクエリでは不発火＝副作用なし。`USE_CLIENT_BOOST=true`（既定OFF・read-only・本番非干渉）。**検証済みの精度施策＝orchestratorで有効化推奨**。`rerank_pool` も `SEARCH_RERANK_POOL_SIZE` で可変化済（未A/B）。
- 残miss: case12「飲料メーカー」（カテゴリ）・case15「BANT A 案件一覧」（集約）。**集約モード(既存)はA/Bで case15 回復せず**（抽出器が当該phrasingに不発火）。→ **検証で効いた精度施策は client-boost(+4pp)のみ**。残2件は固有名詞でなく安価レバー対象外＝gold setはほぼ天井(80-88%)。次の精度ジャンプは #5忠実性(別軸)/multi-query(#6・低ROI)。残フェーズ: Phase 6c（人間ゲート後）、Phase 3（ゲート①）、（任意）複数seed・Phase 5。
- **★⑤ 忠実性 着手＋検証（commit `06dc5ce`＋fix）**: 決定的 citation-validity スコアラー（`faithfulness.py`・捏造引用/無引用を検出・課金0）＋orchestratorが取得chunk_idを捕捉。実測で **引用4/裏付け4/捏造0/validity=1.00**（潤沢クエリ）、**架空クライアント(データ無し)ストレステストも validity=1.00・捏造0**（agentは「データ未取込」と正直に前提訂正・偽前提に引きずられず）＝**orchestratorの引用忠実性は頑健**。途中 regex バグ（markdownバッククォート引用の取りこぼし）を測定が炙り出し修正。⑤-e引用強制promptは不要。残: ⑤-g faithfulnessをeval常設指標化(0)・⑤-f LLM judge claim単位(課金)。
