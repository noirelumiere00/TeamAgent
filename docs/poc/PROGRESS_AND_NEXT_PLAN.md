...
- 残miss: case12「飲料メーカー」（カテゴリ→**multi-query #6** が候補）・case15「BANT A 案件一覧」（集約→**既存 USE_AGGREGATION_MODE** が候補・未検証）。残フェーズ: Phase 6c（人間ゲート後）、Phase 3（ゲート①）、#5忠実性eval、（任意）複数seed・Phase 5。
