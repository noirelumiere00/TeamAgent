...
| **I01a** | 本人特定失敗: 署名済み Slack caller が無い（署名 claim 拒否を含む） | mcp gateway | `caller_claim_rejected` / `identity_spoof_rejected reason=missing_verified_caller` | 管理者へ | OpenClaw の caller-identity plugin / `_user_context` 欠落。Slack 以外の経路から呼んでいないか。**`reason="caller claim request binding does not match"` は「署名した引数 ≠ 実行された引数」**で、2026-09-11 まで unwrap した呼び出しで必ず起きていた（同時刻の `unwrapped tool arguments` と 1:1 で並ぶ）。同時刻に `reconciled unwrapped tool arguments removed=[…]` が出ていれば対策後 |
...
| **P05** | session / channel の束縛が合わない、または**明示的ななりすまし申告**（`_user_context.slack_user_id` に別人／`caller_claim` の持ち込み）。**実測 2 件** | `trusted Slack session or channel binding is missing` / `tool context does not match the bound Slack run` / `declared Slack caller does not match …` / `model-supplied or replayed caller claim is forbidden` | もう一度送ってみる | `id_shape` の `channel:` を見る。`other` なら会話 id を解決できていない（`bind_agent_run` の `resolve=[…]` と突合）。**申告の「欠落」は 2026-09-11 以降 P05 にならない**（欠落は authoritative 値で補われる） |
| **P06** | ツール引数そのものが扱えない形（params がオブジェクトでない／包みが 3 段以上）。**`_user_context` の欠落・非オブジェクトは 2026-09-11 以降ここに落ちない**（申告に依存せず ingress から claim を組む） | `tool params must be a plain object` / `tool arguments are nested deeper than the unwrap limit` | もう一度送ってみる | 直前に `unwrapped tool arguments (…)` が出ているか確認。P06 が残るならモデルが新しい包み方をしている。`discarded non-object declared user_context shape=…` は**拒否ではなく観測**（通っている） |
