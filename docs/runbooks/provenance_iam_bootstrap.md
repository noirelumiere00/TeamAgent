...
- Use a clean reviewed `origin/dev`; local `dev` must equal a fresh remote
  lookup.
...
mutation. Its broad control-plane attachment is ordered after these denials,
and it cannot mutate the bootstrap seed IAM objects, mutate the durable
bootstrap ledger row, or delete the ledger table. Both root paths require MFA
and fixed source identity.
...
Recovery is a separate change with an exact reconciliation manifest. The
one-time tool intentionally contains no force, unlock, reset, state-remove,
import, or retry switch.
