...
Create two purpose-specific secret **containers** through the reviewed external
Secrets Manager bootstrap. Terraform receives only their exact ARNs and never
creates, versions, reads, or stores secret values:
...
4. Prebuild and authorize the connect-web image separately and record its immutable digest; the
   ...
   Create the complete plan with `infra/deploy/terraform_runtime_guard.sh plan`, review the full task/service change and
   ...
   `infra/deploy/terraform_runtime_guard.sh apply`. The apply runs full-payload `pre-register` and a fresh
...
3. Apply that same saved plan once through `infra/deploy/terraform_runtime_guard.sh apply`; a different plan cannot
